package com.chodae.controller.scjinfo;
...
@Tag(name = "신천지 불법사례", description = "신천지 불법사례 페이지 관련 API")
@Slf4j
@RestController
@RequestMapping("/api/scj-info/illegal-cases")
@CrossOrigin(origins = "*")
@RequiredArgsConstructor
public class IllegalCasesController {
    ...
    public ApiResponse<PagedListResponse<ScjInfoResponse>> getList(
        ...
        PagedListResponse<ScjInfoResponse> list = scjInfoService.findAllWithPaging("illegal-cases", pageNumber, itemCount, pageSize, sorting);
        return ApiResponse.success(list);
    }

    @GetMapping("/detail/{id}")
    public ApiResponse<ScjInfoResponse> getDetail(@PathVariable Integer id, Authentication auth) {
        ScjInfoResponse post = scjInfoService.findByIdForBoard("illegal-cases", id);
        if (post == null) {
    ...
    public ApiResponse<ScjInfoResponse> create(@RequestBody ScjInfoCreateRequest request, Authentication auth) {
        ...
        request.setSubMenu("illegal-cases");
    ...
    public ApiResponse<CommentResponse> addComment(@PathVariable Integer id, @RequestBody CommentCreateRequest request, Authentication auth) {
        ...
        ScjInfoResponse post = scjInfoService.findByIdAndUserId(id, userId);
        if (post == null) {
            return ApiResponse.error("글이 존재하지 않거나 접근 권한이 없습니다.");
        }
    ...
    @PutMapping("/edit/{id}")
    public ApiResponse<ScjInfoResponse> edit(@PathVariable Integer id, @RequestBody ScjInfoCreateRequest request, Authentication auth) {
        String userId = getCurrentUserId(auth);
        ...
        request.setUserId(userId);
        request.setSubMenu("illegal-cases");
        return ApiResponse.success("글이 수정되었습니다.", scjInfoService.edit("illegal-cases", id, request));
    }
    ...
    public ApiResponse<CounselingDeleteResponse> deletePost(@PathVariable Integer id, Authentication auth) {
        String userId = getCurrentUserId(auth);
        if (userId == null) return ApiResponse.error("로그인이 필요합니다.");
        return ApiResponse.success("글이 삭제되었습니다.", scjInfoService.deletePostById("illegal-cases", id, userId));
    }
    ...
    public ApiResponse<CommentResponse> updateComment(@PathVariable Integer postId, @PathVariable Integer commentId,
        ...
        request.setUserId(userId);
        request.setSubMenu("illegal-cases");
        return ApiResponse.success("댓글이 수정되었습니다.", scjInfoService.updateComment("illegal-cases", postId, commentId, request));
    }
    ...
    public ApiResponse<CommentDeleteResponse> deleteComment(@PathVariable Integer postId, @PathVariable Integer commentId, Authentication auth) {
        String userId = getCurrentUserId(auth);
        if (userId == null) return ApiResponse.error("로그인이 필요합니다.");
        return ApiResponse.success("댓글이 삭제되었습니다.", scjInfoService.deleteComment("illegal-cases", postId, commentId, userId));
    }
}

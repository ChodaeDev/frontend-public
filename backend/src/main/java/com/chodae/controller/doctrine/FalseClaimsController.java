...
@Tag(name = "거짓 반증", description = "거짓 반증 페이지 관련 API")
@Slf4j
@RestController
@RequestMapping("/api/doctrine/false-claims")
@CrossOrigin(origins = "*")
@RequiredArgsConstructor
public class FalseClaimsController {
    ...
    @GetMapping("/list")
    public ApiResponse<List<DoctrineResponse>> getList(Authentication auth) {
        String userId = getCurrentUserId(auth);
        if (userId == null) {
            return ApiResponse.error("로그인이 필요합니다.");
        }
        List<DoctrineResponse> list = doctrineService.findAll();

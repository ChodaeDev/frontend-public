...
public class DamageCasesController {
    ...
    @GetMapping("/list")
    public ApiResponse<List<WithdrawalResponse>> getList(Authentication auth) {
        String userId = getCurrentUserId(auth);
        if (userId == null) {
            return ApiResponse.error("로그인이 필요합니다.");
        }
        List<WithdrawalResponse> list = withdrawalService.findAll();
    ...
    public ApiResponse<WithdrawalResponse> getDetail(@PathVariable Integer id, Authentication auth) {
        ...
        WithdrawalResponse post = withdrawalService.findByIdAndAuthorId(id, userId);
        if (post == null) {

...
@Tag(name = "법적 자료 모음", description = "법적 자료 모음 페이지 관련 API")
@Slf4j
@RestController
@RequestMapping("/api/doctrine/legal")
@CrossOrigin(origins = "*")
...
public class LegalController {
    ...
    public ApiResponse<PagedListResponse<DoctrineResponse>> getList(
        ...
        PagedListResponse<DoctrineResponse> list = doctrineService.findAllWithPaging("legal", pageNumber, itemCount, pageSize, sorting);
        return ApiResponse.success(list);
    }

    @GetMapping("/detail/{id}")
    public ApiResponse<DoctrineResponse> getDetail(@PathVariable Integer id, Authentication auth) {
        DoctrineResponse post = doctrineService.findByIdForBoard("legal", id);
        if (post == null) {
    ...
    public ApiResponse<DoctrineResponse> edit(@PathVariable Integer id, @RequestBody DoctrineCreateRequest request, Authentication auth) {
        ...
        request.setUserId(userId);
        request.setSubMenu("legal");
        return ApiResponse.success("글이 수정되었습니다.", doctrineService.edit("legal", id, request));
    }
    ...
    public ApiResponse<CounselingDeleteResponse> deletePost(@PathVariable Integer id, Authentication auth) {
        String userId = getCurrentUserId(auth);
        if (userId == null) return ApiResponse.error("로그인이 필요합니다.");
        return ApiResponse.success("글이 삭제되었습니다.", doctrineService.deletePostById("legal", id, userId));
    }
    ...
    public ApiResponse<CommentResponse> updateComment(@PathVariable Integer postId, @PathVariable Integer commentId,
        ...
        request.setUserId(userId);
        request.setSubMenu("legal");
        return ApiResponse.success("댓글이 수정되었습니다.", doctrineService.updateComment("legal", postId, commentId, request));
    }
    ...
    public ApiResponse<CommentDeleteResponse> deleteComment(@PathVariable Integer postId, @PathVariable Integer commentId, Authentication auth) {
        String userId = getCurrentUserId(auth);
        if (userId == null) return ApiResponse.error("로그인이 필요합니다.");
        return ApiResponse.success("댓글이 삭제되었습니다.", doctrineService.deleteComment("legal", postId, commentId, userId));
    }
}

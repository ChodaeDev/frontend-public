...
@Tag(name = "피해 가족 유의사항", description = "피해 가족 유의사항 페이지 관련 API")
@Slf4j
@RestController
@RequestMapping("/api/about/family-tips")
@CrossOrigin(origins = "*")
@RequiredArgsConstructor
public class FamilyTipsController {
    ...
    @GetMapping("/list")
    public ApiResponse<List<AboutResponse>> getList(Authentication auth) {
        String userId = getCurrentUserId(auth);
        if (userId == null) {
            return ApiResponse.error("로그인이 필요합니다.");
        }
        List<AboutResponse> list = aboutService.findAll();
    ...
    public ApiResponse<AboutResponse> getDetail(@PathVariable Integer id, Authentication auth) {
        ...
        AboutResponse post = aboutService.findByIdAndAuthorId(id, userId);
        if (post == null) {

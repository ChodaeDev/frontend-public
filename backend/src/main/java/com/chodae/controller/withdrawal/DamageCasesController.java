...
@Tag(name = "신천지 피해사례", description = "신천지 피해사례 페이지 관련 API")
@Slf4j
@RestController
@RequestMapping("/api/withdrawal/damage-cases")
@CrossOrigin(origins = "*")
@RequiredArgsConstructor
public class DamageCasesController {
    ...
    public ApiResponse<PagedListResponse<WithdrawalResponse>> getList(
        ...
        PagedListResponse<WithdrawalResponse> list = withdrawalService.findAllWithPaging("damage-cases", pageNumber, itemCount, pageSize, sorting);
        return ApiResponse.success(list);
    }

    @GetMapping("/detail/{id}")
    public ApiResponse<WithdrawalResponse> getDetail(@PathVariable Integer id, Authentication auth) {
        WithdrawalResponse post = withdrawalService.findByIdForBoard("damage-cases", id);
        if (post == null) {
    ...
    public ApiResponse<WithdrawalResponse> create(@RequestBody WithdrawalCreateRequest request, Authentication auth) {
        ...
        request.setSubMenu("damage-cases");
    ...
    @PutMapping("/edit/{id}")
    public ApiResponse<WithdrawalResponse> edit(@PathVariable Integer id, @RequestBody WithdrawalCreateRequest request, Authentication auth) {
        String userId = getCurrentUserId(auth);
        ...
        request.setUserId(userId);
        request.setSubMenu("damage-cases");
        return ApiResponse.success("글이 수정되었습니다.", withdrawalService.edit("damage-cases", id, request));
    }
    ...
    public ApiResponse<CounselingDeleteResponse> deletePost(@PathVariable Integer id, Authentication auth) {
        String userId = getCurrentUserId(auth);
        if (userId == null) return ApiResponse.error("로그인이 필요합니다.");
        return ApiResponse.success("글이 삭제되었습니다.", withdrawalService.deletePostById("damage-cases", id, userId));
    }
    ...
    public ApiResponse<CommentResponse> updateComment(@PathVariable Integer postId, @PathVariable Integer commentId,
        ...
        request.setUserId(userId);
        request.setSubMenu("damage-cases");
        return ApiResponse.success("댓글이 수정되었습니다.", withdrawalService.updateComment("damage-cases", postId, commentId, request));
    }
    ...
    public ApiResponse<CommentDeleteResponse> deleteComment(@PathVariable Integer postId, @PathVariable Integer commentId, Authentication auth) {
        String userId = getCurrentUserId(auth);
        if (userId == null) return ApiResponse.error("로그인이 필요합니다.");
        return ApiResponse.success("댓글이 삭제되었습니다.", withdrawalService.deleteComment("damage-cases", postId, commentId, userId));
    }
}
